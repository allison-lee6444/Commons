...
class ProtonmailClient:
    ...
    def login(self, username, password):
        ...
        try:
            ...
            username_input = self.web_driver.find_element(By.ID,
                                                          variables.element_login['username_id'])
            password_input = self.web_driver.find_element(By.ID,
    ...
    def send_mail(self, to, subject, message, as_html=False, attachments=[]):
        ...
        el = self.web_driver.find_element(By.XPATH, '//button[text()="New message"]')
        el.click()
        ...
        # type receivers list
        el = self.web_driver.find_element(By.XPATH, '//input[@placeholder="Email address"]')
        for address in to:
            el.send_keys(address + ";")
            time.sleep(0.2)

        # type subject
        el = self.web_driver.find_element(By.XPATH, '//input[@placeholder="Subject"]')
