...
def generate_invite(cur, target_user_id, invite_sender_id, chatroom_id, uni_id):
    try:
        ...
        AND c.course_id IS NULL and s.uni_id=t.uni_id""",
                    {"sender_id": invite_sender_id, "target_id": target_user_id, "chatroom_id": chatroom_id})
        res = cur.fetchall()[0][0]
        if (res == None or res == []):
            raise HTTPException(
                status_code=500,
        ...
        cur.execute(
            "INSERT INTO invite values(%(invite_id)s, %(chatroom_id)s, %(invite_sender_id)s, %(target_user_id)s, %(uni_id)s)",
            {"invite_id": invite_id, "chatroom_id": chatroom_id, "invite_sender_id": invite_sender_id,
             "target_user_id": target_user_id, "uni_id": uni_id})
...
def accept_invite(cur, invite_id, target_user_id):
    try:
        ...
        cur.execute("SELECT i.chatroom_id, c.chatroom_name "
                    "FROM invite i JOIN (chatroom c JOIN student s on c.uni_id = s.uni_id) on i.chatroom_id = c.id"
                    " WHERE invite_id = %(invite_id)s AND target_user_id = %(target_user_id)s",
                    {"invite_id": invite_id, "target_user_id": target_user_id})
        chatroom_id, chatroom_name = cur.fetchall()[0]
        ...
        cur.execute(
            "INSERT INTO in_chatroom(student_id, uni_id, chatroom_id) VALUES(%(target_user_id)s, (SELECT uni_id FROM chatroom WHERE id = %(chatroom_id)s), %(chatroom_id)s)",
            {"target_user_id": target_user_id, "chatroom_id": chatroom_id})

        return chatroom_id, chatroom_name
    except BaseException as e:
        print(f'Exception: {e}')
        raise HTTPException(
            status_code=500,
...
def getChatrooms(cur, student_id, uni_id):
    try:
        cur.execute(
            "SELECT chatroom.id,chatroom.chatroom_name,chatroom.course_id IS NULL FROM in_chatroom LEFT JOIN chatroom ON "
